...
export const LandingPage = ({ setActiveTab, onOpenPhotoSearch }) => {
  ...
  return (
    <div className="space-y-10 sm:space-y-20 pb-12 sm:pb-20">
      {/* ============================================================
          HERO SECTION
          ============================================================ */}
      <section className="relative pt-4 sm:pt-10 pb-8 sm:pb-16 overflow-hidden">
        ...
        <div className="text-center max-w-4xl mx-auto space-y-5 sm:space-y-8 animate-slideUp px-1">
          {/* Hackathon badge */}
          <div className="inline-flex items-center space-x-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full glass-panel border border-slate-700/60 shadow-lg max-w-full truncate">
            <span className="flex h-2 w-2 relative shrink-0">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500" />
            </span>
            <span className="text-[11px] sm:text-xs font-semibold text-slate-300 truncate">
              Google for Developers × PromptWars • Build with AI
            </span>
          </div>

          <h1 className="text-3xl sm:text-5xl md:text-7xl font-black tracking-tight text-white leading-[1.15]">
            Lost Something? <br />
            <span className="text-gradient-primary animate-floatY inline-block mt-1">
              AI's Got Your Back.
            </span>
          </h1>

          <p className="text-sm sm:text-lg md:text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed font-medium px-2">
            Report what you lost or found, and let AI find the connection in seconds.
          </p>

          {/* Primary CTAs — Large, Easy Single-Hand Mobile Tap Targets */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4 pt-2 max-w-lg mx-auto">
            <button
              onClick={() => setActiveTab('report')}
              className="min-h-[58px] w-full flex items-center justify-between px-6 py-4 rounded-2xl text-base font-extrabold text-white shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] border border-rose-500/40"
              style={{ background: 'linear-gradient(135deg, #e11d48 0%, #dc2626 100%)', boxShadow: '0 8px 24px -6px rgba(225,29,72,0.4)' }}
            >
              <div className="flex items-center space-x-3">
                ...
                <div className="text-left">
                  <p className="text-sm font-black leading-tight">Report Lost Item</p>
                  <p className="text-[11px] text-rose-100 font-medium opacity-90">Find your missing item</p>
                </div>
              </div>
            ...
            <button
              onClick={() => setActiveTab('report')}
              className="min-h-[58px] w-full flex items-center justify-between px-6 py-4 rounded-2xl text-base font-extrabold text-white shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] border border-emerald-500/40"
              style={{ background: 'linear-gradient(135deg, #059669 0%, #0d9488 100%)', boxShadow: '0 8px 24px -6px rgba(5,150,105,0.4)' }}
            >
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center">
                ...
                <div className="text-left">
                  <p className="text-sm font-black leading-tight">Report Found Item</p>
                  <p className="text-[11px] text-emerald-100 font-medium opacity-90">Return an item (+Karma)</p>
                </div>
              </div>
              <ArrowRight className="w-5 h-5 opacity-80" />
            </button>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5 pt-1">
            <button
              ...
              className="btn-ghost min-h-[44px] px-4 py-2.5 rounded-xl flex items-center space-x-2 hover:border-cyan-500/40"
            >
              <Camera className="w-4 h-4 text-cyan-400" />
              <span className="text-cyan-300 text-xs font-bold">Search by Photo</span>
            </button>
            <button
              onClick={() => setActiveTab('browse')}
              className="btn-ghost min-h-[44px] px-4 py-2.5 rounded-xl flex items-center space-x-2"
            >
              <Compass className="w-4 h-4 text-campus-400" />
              <span className="text-xs font-bold">Explore Catalog</span>
            </button>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto mt-10 sm:mt-16">
          <div className="glass-panel rounded-2xl p-4 sm:p-5 border border-slate-800/60">
        ...
        {/* KPI Stats */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 max-w-5xl mx-auto mt-6 sm:mt-10 stagger-children">
          {stats.map((s, idx) => {
            const Icon = s.icon;
            return (
              <div key={idx} className="glass-card rounded-2xl p-4 sm:p-5 border border-slate-800/60 text-center group">
                <div className={`w-9 h-9 sm:w-10 sm:h-10 mx-auto rounded-xl bg-gradient-to-r ${s.color} flex items-center justify-center text-white mb-2 sm:mb-3 shadow-md group-hover:scale-110 transition-transform`}>
                  <Icon className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
                <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tight">{s.value}</h3>
                <p className="text-xs font-bold text-slate-200 mt-0.5 sm:mt-1">{s.label}</p>
                <p className="text-[11px] sm:text-xs text-slate-400 mt-0.5 truncate">{s.sub}</p>
              </div>
            );
          })}
        </div>
      ...
      <section className="max-w-5xl mx-auto">
        <div className="rounded-3xl overflow-hidden border border-slate-800/60 bg-grid-pattern relative">
          ...
          <div className="p-4 sm:p-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 pb-5 border-b border-slate-800/60 relative">
              <div>
      ...
      <section className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 stagger-children">
          {innovations.map((item, idx) => {
            ...
            return (
              <div key={idx} className="glass-card rounded-2xl p-5 sm:p-6 border border-slate-800/60 flex flex-col justify-between group">
                <div>
                  <div className={`w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-r ${item.color} flex items-center justify-center text-white mb-3 sm:mb-4 shadow-lg group-hover:scale-110 transition-transform`}>
                    <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                  </div>
                  <h4 className="text-base font-bold text-white mb-1.5">{item.title}</h4>
                  <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{item.desc}</p>
                </div>
                <div className="mt-4 pt-3 border-t border-slate-800/60 text-xs font-semibold text-emerald-400 flex items-center gap-1.5">
      ...
      {/* ============================================================
          ...
          ============================================================ */}
      <section className="max-w-5xl mx-auto">
        <div className="glass-panel rounded-3xl p-5 sm:p-12 border border-slate-800/60 relative overflow-hidden bg-grid-pattern">
          <div className="absolute inset-0 bg-gradient-to-br from-campus-950/30 via-transparent to-ai-purple/10 pointer-events-none" />

          <h2 className="text-xl sm:text-3xl font-black text-white text-center mb-8 sm:mb-12 relative">
            How It Works in <span className="text-gradient-primary">3 Quick Steps</span>
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-8 relative stagger-children">
            {[
              ...
              <div key={idx} className="relative p-5 sm:p-7 rounded-2xl bg-slate-900/50 border border-slate-800/60 hover:border-slate-700/80 text-center group transition-all hover:-translate-y-1 duration-300">
                <div className={`w-14 h-14 sm:w-16 sm:h-16 mx-auto rounded-2xl bg-gradient-to-r ${step.color} flex items-center justify-center text-white text-2xl font-black mb-4 sm:mb-5 shadow-lg group-hover:scale-110 transition-transform`}>
                  {step.emoji}
                </div>
                <h4 className={`text-base sm:text-lg font-extrabold mb-1.5 ${step.accent}`}>{step.title}</h4>
          ...
          <div className="mt-8 sm:mt-10 text-center relative">
            <button
              onClick={() => setActiveTab('report')}
              className="btn-primary min-h-[52px] w-full sm:w-auto inline-flex items-center justify-center space-x-2"
            >
